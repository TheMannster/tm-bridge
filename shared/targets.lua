...
function createEntityTarget(entity, opts, dist)
    ...
    if Config.System.DontUseTarget or (not isStarted(OXTargetExport) and not isStarted(QBTargetExport)) then
        ...
        for _, target in pairs(TextTargets) do
            if target.coords and #(target.coords - entityCoords) < 0.01 then
        ...
        if existingTarget then
            for i = 1, #opts do
                local key = KEY_TABLE[#existingTarget.options + i] or KEY_TABLE[1]
                opts[i].key = key
                existingTarget.buttontext[#existingTarget.buttontext + 1] = " ~b~[~w~" .. (Keys[key] or "UNK") .. "~b~] ~w~" .. opts[i].label
            ...
            local tempText = {}
            for i = 1, #opts do
                opts[i].key = KEY_TABLE[i] or KEY_TABLE[1]
                tempText[#tempText + 1] = " ~b~[~w~" .. (Keys[opts[i].key] or "UNK") .. "~b~] ~w~" .. opts[i].label
            end
            TextTargets[entity] = {
...
function createBoxTarget(data, opts, dist)
    local zoneName = data[1]
    local zoneCoords = data[2]
    local zoneWidth = data[3]
    ...
    if Config.System.DontUseTarget or (not isStarted(OXTargetExport) and not isStarted(QBTargetExport)) then
        ...
        local existingTarget = nil
        for _, target in pairs(TextTargets) do
            if target.coords and #(target.coords - zoneCoords) < 0.01 then
                existingTarget = target
                break
            end
        end
        ...
        return zoneName
    elseif isStarted(OXTargetExport) then
        debugPrint("^6Bridge^7: ^2Creating new ^3Box^2 target with ^6"..OXTargetExport.." ^2for zone ^7"..zoneName)
        local optionsForOx = {}
        for i = 1, #opts do
            optionsForOx[i] = {
                icon = opts[i].icon,
        ...
        local boxHeight = zoneParams.useZ and (zoneParams.maxZ - zoneParams.minZ) or 1.0 -- Default height or calculated
        local centerZ = zoneParams.useZ and (zoneParams.minZ + boxHeight / 2) or zoneCoords.z
        
        local target = exports[OXTargetExport]:addBoxZone({
            name = zoneName, -- OX Target uses name for identification
            coords = vec3(zoneCoords.x, zoneCoords.y, centerZ),
            size = vec3(zoneWidth, zoneLength, boxHeight), -- x:width, y:length, z:height
            rotation = zoneParams.heading,
            debug = zoneParams.debugPoly,
            options = optionsForOx
        })
        boxTargets[#boxTargets + 1] = {name = zoneName, id = target} -- Store by name and ox_target id
        return target
    elseif isStarted(QBTargetExport) then
        debugPrint("^6Bridge^7: ^2Creating new ^3Box^2 target with ^6"..QBTargetExport.." ^2for zone ^7"..zoneName)
        local options = { options = opts, distance = dist }
        local target = exports[QBTargetExport]:AddBoxZone(zoneName, zoneCoords, zoneLength, zoneWidth, zoneParams, options)
        boxTargets[#boxTargets + 1] = {name = zoneName} -- Store by name
        return zoneName
    end
end
...
function createCircleTarget(data, opts, dist)
    local zoneName = data.name or data[1]
    local zoneCoords = data.coords or data[2]
    ...
    if Config.System.DontUseTarget then
        ...
        local existingTarget = nil
        for _, target in pairs(TextTargets) do
            if #(target.coords - zoneCoords) < 0.01 then
                existingTarget = target
                break
            end
        end
        ...
        return zoneName
    elseif isStarted(OXTargetExport) then
        debugPrint("^6Bridge^7: ^2Creating new ^3Circle ^2target with ^6"..OXTargetExport.." ^2for zone ^7"..zoneName)
        local optionsForOx = {}
        for i = 1, #opts do
            optionsForOx[i] = {
                icon = opts[i].icon,
        ...
        circleTargets[#circleTargets + 1] = {name = zoneName, id = target}
        return target
    elseif isStarted(QBTargetExport) then
        debugPrint("^6Bridge^7: ^2Creating new ^3Circle ^2target with ^6"..QBTargetExport.." ^2for zone ^7"..zoneName)
        local options = { options = opts, distance = dist }
        local target = exports[QBTargetExport]:AddCircleZone(zoneName, zoneCoords, zoneRadius, zoneParams, options)
        circleTargets[#circleTargets + 1] = {name = zoneName}
        return zoneName
    end
end
...
onResourceStop(function()
    ...
    for i = 1, #boxTargets do
        if isStarted(OXTargetExport) then
            exports[OXTargetExport]:removeZone(boxTargets[i].id or boxTargets[i].name, true)
        elseif isStarted(QBTargetExport) then
    ...
    for i = 1, #circleTargets do
        if isStarted(OXTargetExport) then
            exports[OXTargetExport]:removeZone(circleTargets[i].id or circleTargets[i].name, true)
        elseif isStarted(QBTargetExport) then
